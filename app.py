...
st.set_page_config(page_title="BOM Tool v4.2", layout="wide")
...
try:
    ...
    # Create a dictionary for quick detail lookups
    item_details = df_master.set_index('Part No.').to_dict('index')

    # Standardize links (First 3 columns)
    ...
    # We find all "Make" items in the Master. These represent assemblies/saleable items.
    make_items = df_master[df_master['Make/Buy'] == 'Make'].copy()
    
    # Create a display string: "Part No | Description"
    make_items['Dropdown_Label'] = make_items['Part No.'] + " | " + make_items['Part Description'].fillna("")
    
    # Sort and create the selection list
    sku_options = sorted(make_items['Dropdown_Label'].unique())

    st.sidebar.header("Navigation")
    search_query = st.sidebar.text_input("Search for Part or Name", "")
    
    # Filter dropdown based on sidebar search
    if search_query:
        sku_options = [opt for opt in sku_options if search_query.upper() in opt.upper()]

    selected_label = st.selectbox(f"Select Saleable SKU ({len(sku_options)} items)", ["-- Select --"] + sku_options)

    if selected_label != "-- Select --":
        # Extract the Part No from the selection (it's the part before the '|')
        selected_sku = selected_label.split(" | ")[0].strip()
        selected_desc = item_details.get(selected_sku, {}).get('Part Description', "No Description Found")
        ...
        if waterfall:
            ...
            # KPI Metrics
            m1, m2 = st.columns(2)
            m1.metric("Total Roll-up Cost", f"${df_wf['Ext. Cost'].sum():,.2f}")
            ...
            st.download_button("Export CSV", csv, f"{selected_sku}_BOM.csv", "text/csv")
        else:
            st.warning("⚠️ This item is marked as 'Make' but has no children defined in the BOM Links file.")
