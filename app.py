...
st.set_page_config(page_title="BOM Tool v5.1", layout="wide")
...
try:
    ...
    # Clean header whitespace to prevent the "Double Space" error
    df_sku_list.columns = [c.strip() for c in df_sku_list.columns]
    # Standardize Cladding header specifically
    ...
    ui_option = st.sidebar.radio(
        ...
        ["Option 1: Saleable SKUs (0 Prefix)", "Option 2: Base Assemblies", "Option 3: Countertop Assemblies", "Option 4: Cladding Assemblies", "Option 5: Finish Kits"]
    )
    ...
    if selected_label != "-- Select --":
        ...
        if waterfall:
            ...
            st.warning("⚠️ No components found in BOM Links for this ID.")
